...
class TagCrudController extends AbstractCrudController
{
    ...
    {
        return $crud
        ->overrideTemplate('crud/detail', 'admin/tag/tag_detail.html.twig')
        ->overrideTemplate('crud/index', 'admin/tag/index.html.twig')
    ...
    public function archiveTag(AdminContext $context): Response
    {
        $tag = $context->getEntity()->getInstance();
        if ($tag instanceof Tag) {
            $tag->setDeletedAt(new \DateTimeImmutable());
            $this->entityManager->flush();
            $this->addFlash('success', sprintf('Tag "%s" was archived.', $tag->getName()));
        }
    ...
    public function restoreTag(AdminContext $context): Response
    {
        $tag = $context->getEntity()->getInstance();
        if ($tag instanceof Tag) {
            $tag->setDeletedAt(null);
            $this->entityManager->flush();
            $this->addFlash('success', sprintf('Tag "%s" was restored.', $tag->getName()));
        }
